...
def prison_after_n_days(cells, n)
  ...
  n.times do |i|
    (1..6).each do |col|
      result[col] = cells[col - 1] == cells[col + 1] ? 0 : 1
...
describe "#prison_after_n_days" do
  it 'gives state of prison after n days' do
    expect(prison_after_n_days([1,0,0,0,0,1,0,0], 1)).to eq [0,1,0,0,1,0,1,0]
    expect(prison_after_n_days([1,1,1,0,1,1,1,1], 2)).to eq [0,0,0,0,0,1,1,0]
